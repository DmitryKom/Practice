...
class Board:
    ...
    def place_computer_ships(self):
        for ship, size in self.ships.items():
            ships_placed = 0  # Track the number of placed ships for this type

            while ships_placed < self.ship_counts[ship]:
    ...
    def check_hit(self, x, y):
        if self.board[y][x] == '':
            self.board[y][x] = 'O'
            return False
        elif self.board[y][x] != 'X' and self.board[y][x] != 'O':
...
class BattleshipGame(wx.Frame):
    def __init__(self, parent):
        ...
        self.update_grids()
        self.computer_board.place_computer_ships()
        self.last_hit = None
